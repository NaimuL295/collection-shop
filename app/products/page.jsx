              ...
              <Link href={`/products/${product._id}`} key={product._id} className="block group">
                <div className="relative bg-white rounded-[18px] shadow-[0_4px_15px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_25px_rgba(0,0,0,0.12)] transition-all duration-500 border border-gray-100 overflow-hidden">
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 z-20 bg-white/90 backdrop-blur-sm p-2.5 rounded-full shadow-md hover:scale-110 transition-all"
                  >
                    <Heart className={`w-5 h-5 ${isInWishlist(product._id) ? "text-red-500 fill-red-500" : "text-gray-400 group-hover:text-red-500"}`} />
                  </button>
  ...
  {product.images?.[0]?.url ? (
    ...
    <div className="flex items-center justify-center h-full text-gray-400">
      <p>No Image</p>
                  ...
                  {/* Details */}
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-900 mb-3 text-[15px] leading-snug line-clamp-2">{product.name}</h3>

                    <div className="flex items-center justify-between mb-5">
                      <div className="flex items-center gap-2">
                        {product.offerprice ? (
                          <>
                            <p className="text-xl font-bold text-gray-900">৳{product.offerprice}</p>
                            <p className="text-sm text-gray-400 line-through">৳{product.price}</p>
                          </>
                        ) : (
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full ${product.stock > 0 ? "text-green-700 bg-green-100" : "text-red-700 bg-red-100"}`}>
                    ...
                    <button
                      ...
                      disabled={product.stock <= 0}
                      className={`w-full py-3.5 rounded-xl text-sm font-medium transition-all duration-300 shadow-md flex items-center justify-center gap-2 ${product.stock > 0 ? "bg-blue-600 text-white hover:bg-blue-700 hover:-translate-y-[2px] active:scale-95 hover:shadow-xl" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
                    >
